...
int16_t	philo_eat(t_philo *philo)
{
		pthread_mutex_lock(philo->fork_l);
			pthread_mutex_lock(&philo->start_mutex);
				printf("%ld %d has taken fork_l: %p\n", time_ellapsed_in_ms(philo->start_time, get_time()), philo->philo_id + 1, philo->fork_l);
			pthread_mutex_unlock(&philo->start_mutex);

		pthread_mutex_lock(philo->fork_r);
			pthread_mutex_lock(&philo->start_mutex);
				printf("%ld %d has taken fork_r: %p\n", time_ellapsed_in_ms(philo->start_time, get_time()), philo->philo_id + 1, philo->fork_r);
			pthread_mutex_unlock(&philo->start_mutex);
